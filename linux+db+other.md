...
- 首次配对确保手机之前并没有进行连接过或者删除设备。删除设备的具体操作：控制面板-硬件和声音-设备和打印机-删除原先的设备删除
- 手机搜索笔记本蓝牙，并进行配对，对于pin码直接yes就可以了，配对完成后就连接成功了
- 注：笔记本连接成功后，通知中心的状态仍然是not  connected，
- 注：传输文件的时候 setting-设备-蓝牙和其他设备-右侧发送和接收文件通过蓝牙