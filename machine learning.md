...
对于二分类问题：常用的是精确率（precision）和召回率（recall），所分类的情况有四类：将正类判定为正类（TP， true  positive），N（FN，False negative），将负类判定为负类（TN），将负类判定为正类。（FN）。这个可以用图像进行表示
...
ensemble就是把几种机器学习的算法组合在一起或者是把一种算法的不同参数组合到一起，基本分类为平均方法和提升方法。集成方法出名是bagging，bagging的典型代表是随机森林，提升方法boosting，boosting 就是‘三个臭皮匠顶个诸葛亮’的道理，boosting的典型代表是boosting和xgboost
